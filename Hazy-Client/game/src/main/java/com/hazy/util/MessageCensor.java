...
public final class MessageCensor {
    ...
    public static void load(Archive archive) {
        Buffer fragments = new Buffer(
                archive.get("fragmentsenc.txt"));
        Buffer bad = new Buffer(archive.get("badenc.txt"));
        Buffer domain = new Buffer(archive.get("domainenc.txt"));
        Buffer tldlist = new Buffer(archive.get("tldlist.txt"));
    ...
    private static void decodeFragmentsEnc(Buffer stream) {
        ...
        for (int i = 0; i < fragments.length; i++)
            fragments[i] = stream.readUnsignedShort();
    }
