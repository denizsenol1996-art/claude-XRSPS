...
public class ExpCounter {
    ...
    public static void addXP(int skill, int xp, boolean increment) {
        if (skill == 99) {
            ...
            if (xp != 0) {
                if (currentGain != null && Math.abs(currentGain.getY() - START) <= getSize(Client.instance.setting.counter_size).base_char_height) {
                    currentGain.xp += xp;
        ...
        if (!GAINS.isEmpty()) {
            ...
            while (gained.hasNext()) {
                ...
                if (gain.getY() > STOP) {
                    ...
                    getSize(Client.instance.setting.counter_size).draw(drop, x + dx + 2, (int) (gain.getY() + y) + 14, Client.instance.setting.counter_color, 0);
                }
            }
        }
    }
