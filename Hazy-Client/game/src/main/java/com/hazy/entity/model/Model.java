...
public class Model extends Renderable implements RSModel {
    ...
    private Model(int modelId, boolean isItem) {
    ...
    public static Model get(int file, boolean item) {
        ...
        if (header == null) {
            ...
            return new Model(file, item);
        }
    }

    public static boolean cached(int file) {
        if (modelHeaders == null) return false;
    ...
    public Model(boolean colorFlag, boolean alphaFlag, boolean animated, boolean textureFlag, Model model) {
        ...
        if (colorFlag) {
            ...
            System.arraycopy(model.repeatTexture, 0, repeatTexture, 0, trianglesCount);
        }
    ...
    public void replace(Model model, boolean replaceAlpha) {
        verticesCount = model.verticesCount;
        trianglesCount = model.trianglesCount;
        texturesCount = model.texturesCount;
    ...
    public void generateBones() {
        if (vertexData != null) {
            ...
            groupedTriangleLabels = new int[j + 1][];
            for (int k1 = 0; k1 <= j; k1++) {
                groupedTriangleLabels[k1] = new int[ai[k1]];
                ai[k1] = 0;
            }
            for (int j2 = 0; j2 < verticesCount; j2++) {
                int l2 = vertexData[j2];
                groupedTriangleLabels[l2][ai[l2]++] = j2;
            }
            vertexData = null;
        }
    ...
    private void transform(int animationType, int[] skinArray, int x, int y, int z) {
        ...
        if (animationType == 0) {
            ...
            for (int k2 = 0; k2 < length; ++k2) {
                ...
                if (skin >= this.groupedTriangleLabels.length) continue;
                int[] group = this.groupedTriangleLabels[skin];
                for (int i5 = 0; i5 < group.length; ++i5) {
                    int offset = group[i5];
                    xAnimOffset += this.verticesX[offset];
                    yAnimOffset += this.verticesY[offset];
                    zAnimOffset += this.verticesZ[offset];
                    ++i;
                }
            }
            if (i > 0) {
                xAnimOffset = xAnimOffset / i + x;
                yAnimOffset = yAnimOffset / i + y;
                zAnimOffset = zAnimOffset / i + z;
                return;
            }
            xAnimOffset = x;
            yAnimOffset = y;
        ...
        if (animationType == 2) {
            for (int l1 = 0; l1 < length; ++l1) {
                ...
                for (int j4 = 0; j4 < group.length; ++j4) {
                    ...
                    this.verticesX[n] = this.verticesX[n] - xAnimOffset;
                    int n4 = offset;
                    this.verticesY[n4] = this.verticesY[n4] - yAnimOffset;
                    int n5 = offset;
                    this.verticesZ[n5] = this.verticesZ[n5] - zAnimOffset;
                    int xOff = (x & 0xFF) * 8;
                    int yOff = (y & 0xFF) * 8;
                    int zOff = (z & 0xFF) * 8;
                    if (zOff != 0) {
                    ...
                    int n6 = offset;
                    this.verticesX[n6] = this.verticesX[n6] + xAnimOffset;
                    int n7 = offset;
                    this.verticesY[n7] = this.verticesY[n7] + yAnimOffset;
                    int n8 = offset;
                    this.verticesZ[n8] = this.verticesZ[n8] + zAnimOffset;
                }
            }
            return;
        }
    ...
    public void interpolate(int frameId) {
        try {
            if (this.groupedTriangleLabels == null) {
                return;
            }
            if (frameId == -1) {
                return;
            }
            NormalFrame frame = (NormalFrame) SeqFrame.getFrame(frameId);
            if (frame == null) {
                return;
            }
            ...
            for (int k = 0; k < frame.translatorCount; ++k) {
                ...
                this.transform(base.types[l], base.frameMaps[l], frame.translatorX[k], frame.translatorY[k], frame.translatorZ[k]);
            }
            this.resetBounds();
            this.invalidate();
        } catch (Exception e) {
    ...
    public void playSkeletal(SeqDefinition seqDefinition, int tick) {
        ...
        SkeletalFrame skeletalFrameset = SkeletalFrame.getSkeletalFrame(skeletalId);
        if (skeletalFrameset == null) {
            return;
        }
    ...
    public void transformSkeletal(SkeletalFrame skeletalFrame, int tick) {
        ...
        animationBoneWrapper.update(skeletalFrame, tick);
        this.transformSkeletal(animationBoneWrapper, skeletalFrame.getFid(), skeletalFrame);
    ...
    private void transformVertex(int vertex, Matrix4f bm, SkeletalFrame skeletalFrame) {
        transformVertex(this, vertex, bm, skeletalFrame);
    }

    public static void transformVertex(Model model, int vertex, Matrix4f bm, SkeletalFrame skeletalFrame) {
        int oldX = model.verticesX[vertex];
        int oldY = model.verticesY[vertex];
        int oldZ = model.verticesZ[vertex];
        float x = oldX;
        float y = -oldY;
        float z = -oldZ;
        float w = 1.0f;
        ...
        model.verticesX[vertex] = newX;
        model.verticesY[vertex] = newY;
        model.verticesZ[vertex] = newZ;
    }

    private void transformSkeletal(AnimationBoneWrapper animBoneWrapper, int frameId, SkeletalFrame skeletalFrame) {
        if (this.skeletalBones == null) {
            System.err.println("bro? null for " + frameId + " abw = " + animBoneWrapper);
            return;
        }
        Matrix4f fbm = new Matrix4f();
        Matrix4f sm = new Matrix4f();
        Matrix4f cbm = new Matrix4f();
        for (int vertex = 0; vertex < this.verticesCount; ++vertex) {
            ...
            for (int i = 0; i < bonesForVertex.length; ++i) {
                ...
                AnimationBone bone = animBoneWrapper.getAB(bi);
            ...
            this.transformVertex(vertex, fbm, skeletalFrame);
        }
    }
    ...
    public void transformSkeletalMultiple(FrameBase base, SkeletalFrame skeletalFrame, int tick, boolean[] mask, boolean state, boolean hasSkeletalAnimations) {
        AnimationBoneWrapper boneAnimationWrapper = base.getAbw();
        if (boneAnimationWrapper != null) {
            boneAnimationWrapper.update(skeletalFrame, tick, mask, state);
            if (hasSkeletalAnimations) {
                this.transformSkeletal(boneAnimationWrapper, skeletalFrame.getFid(), skeletalFrame);
            }
        }
        if (!state && skeletalFrame.hasAlphaTransforms()) {
            this.applyAlphaTransforms(skeletalFrame, tick);
        }
    }

    private void applyAlphaTransforms(SkeletalFrame skaFSet, int tick) {
        FrameBase base = skaFSet.getFrameBase();
    ...
    public void mix(int[] mask, int secondaryId, int primaryId) {
        if (primaryId == -1) {
            return;
        }
        if (mask == null || secondaryId == -1) {
            this.interpolate(primaryId);
            return;
        }
        NormalFrame primaryFrame = (NormalFrame) SeqFrame.getFrame(primaryId);
        if (primaryFrame == null) {
            return;
        }
        NormalFrame secondaryFrame = (NormalFrame) SeqFrame.getFrame(secondaryId);
        if (secondaryFrame == null) {
            this.interpolate(primaryId);
            return;
        }
        FrameBase frameBase = primaryFrame.getFrameBase();
        xAnimOffset = 0;
        yAnimOffset = 0;
        zAnimOffset = 0;
        int l = 0;
        int i1 = mask[l++];
        for (int j1 = 0; j1 < primaryFrame.translatorCount; ++j1) {
            int k1 = primaryFrame.indexFrameIds[j1];
            while (k1 > i1) {
                i1 = mask[l++];
            }
            if (k1 == i1 && frameBase.types[k1] != 0) continue;
            this.transform(frameBase.types[k1], frameBase.frameMaps[k1], primaryFrame.translatorX[j1], primaryFrame.translatorY[j1], primaryFrame.translatorZ[j1]);
        }
        xAnimOffset = 0;
        yAnimOffset = 0;
        zAnimOffset = 0;
        l = 0;
        i1 = mask[l++];
        for (int l1 = 0; l1 < secondaryFrame.translatorCount; ++l1) {
            int i2 = secondaryFrame.indexFrameIds[l1];
            while (i2 > i1) {
                i1 = mask[l++];
            }
            if (i2 != i1 && frameBase.types[i2] != 0) continue;
            this.transform(frameBase.types[i2], frameBase.frameMaps[i2], secondaryFrame.translatorX[l1], secondaryFrame.translatorY[l1], secondaryFrame.translatorZ[l1]);
        }
        this.resetBounds();
    }
    ...
    public void doppelRecolor(int j) {
        if (colors == null)
            colors = new short[trianglesCount];

        for (int k = 0; k < trianglesCount; k++) {
            colors[k] = (short) j;
            triangleAlpha[k] = (byte) 100;
        }
    }
    ...
    public final void drawFrustum(int var1, int var2, int var3, int var4, int var5, int var6, int var7) {
        ...
        int var8 = Rasterizer3D.originViewX;
        int var9 = Rasterizer3D.originViewY;
        int var10 = SINE[var1];
        ...
        for (int var19 = 0; var19 < this.verticesCount; ++var19) {
            ...
            vertexScreenX[var19] = var8 + var20 * Rasterizer3D.originViewX / var22;
            vertexScreenY[var19] = var9 + var23 * Rasterizer3D.originViewY / var22;
            if (this.texturesCount > 0) {
                vertexMovedX[var19] = var20;
                vertexMovedY[var19] = var23;
                vertexMovedZ[var19] = var22;
            }
        }
        ...
        if (callbacks == null || !callbacks.drawFace(this, face)) {
            ...
            if (materials != null && materials[face] != -1) {
                ...
                Rasterizer3D.drawFlatTriangle(
                    vertexScreenY[triX], vertexScreenY[triY], vertexScreenY[triZ],
                    vertexScreenX[triX], vertexScreenX[triY], vertexScreenX[triZ],
                    modelColors[colorsX[face]], overrideHue, overrideSaturation, overrideLuminance, overrideAmount);
                ...
                if (type == 1) {
                    Rasterizer3D.drawFlatTriangle(vertexScreenY[triX], vertexScreenY[triY],
                        vertexScreenY[triZ], vertexScreenX[triX], vertexScreenX[triY],
                        vertexScreenX[triZ], modelColors[colorsX[face]], overrideHue, overrideSaturation, overrideLuminance, overrideAmount);
                }
            }
            ...
            if (counter == 3) {
                ...
                if (materials != null && materials[triangle] != -1) {
                    ...
                    if (drawType == 0) {
                        ...
                        Rasterizer3D.drawFlatTriangle(yA, yB, yC, xA, xB, xC, modelColors[colorsX[triangle]], overrideHue, overrideSaturation, overrideLuminance, overrideAmount);
                    }
                }
            }
            if (counter == 4) {
                ...
                if (materials != null && materials[triangle] != -1) {
                    ...
                    if (drawType == 1) {
                        ...
                        Rasterizer3D.drawFlatTriangle(yA, yB, yC, xA, xB, xC, l8, overrideHue, overrideSaturation, overrideLuminance, overrideAmount);
                        Rasterizer3D.drawFlatTriangle(yA, yC, yPosition[3], xA, xC, xPosition[3], l8, overrideHue, overrideSaturation, overrideLuminance, overrideAmount);
                    }
                }
            }
        }
    ...
    Model buildSharedModel(boolean var1, Model var2, byte[] var3) {
        ...
        if (var1) {
            var2.triangleAlpha = this.triangleAlpha;
        } else {
            var2.triangleAlpha = var3;
            if (this.triangleAlpha == null) {
                for (var4 = 0; var4 < this.trianglesCount; ++var4) {
                    var2.triangleAlpha[var4] = 0;
                }
            } else {
                for (var4 = 0; var4 < this.trianglesCount; ++var4) {
                    var2.triangleAlpha[var4] = this.triangleAlpha[var4];
                }
            }
        }


        var2.repeatTexture = this.repeatTexture;
    ...
    static byte[] Model_sharedSpotAnimationModeltriangleAlpha = new byte[1];

    static Model Model_sharedSpotAnimationModel = new Model();

    public Model toSharedSpotAnimationModel(boolean var1) {
        if (!var1 && Model_sharedSpotAnimationModeltriangleAlpha.length < this.trianglesCount) {
            Model_sharedSpotAnimationModeltriangleAlpha = new byte[this.trianglesCount + 100];
        }

        return this.buildSharedModel(var1, Model_sharedSpotAnimationModel, Model_sharedSpotAnimationModeltriangleAlpha);
    }
