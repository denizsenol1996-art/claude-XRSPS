...
import static com.hazy.Client.spriteCache;
...
public class AdvancedFont extends Rasterizer2D {
    ...
    public AdvancedFont(boolean monospace, String name, Archive archive) {
        ...
        idx.pos = data.readUnsignedShort() + 4;
        int pos = idx.readUnsignedByte();
        if (pos > 0) {
            idx.pos += 3 * (pos - 1);
        }
        for (int index = 0; index < 256; index++) {
            kerning_x[index] = idx.readUnsignedByte();
            kerning_y[index] = idx.readUnsignedByte();
            int width = glyph_width[index] = idx.readUnsignedShort();
            int height = glyph_height[index] = idx.readUnsignedShort();
    ...
    public AdvancedFont(boolean monospace, String name) throws IOException {
        ...
        Buffer data = new Buffer(ByteStreams.toByteArray(in));
   ...
   /* public static int method1014(byte[][] is, byte[][] is_27_, int[] is_28_, int[] is_29_, int[] is_30_, int i, int i_31_) {//unused
                        ...
                        if (effect.startsWith(set_image)) {
                            try {
                                ...
                                if (opacity == 256) {
                                    ...
                                    icon.drawSprite1(x, (y + base_char_height - height_offset), opacity);
                                }
                                x += icon.width;
                                unknown = -1;
                        ...
                        if (effect.startsWith(set_image)) {
                            try {
                                ...
                                if (opacity == 256) {
                                    ...
                                    chat.drawSprite1(x + x_translate_offset, (y + base_char_height - icon_offset_y + y_translate_offset), opacity);
                                }
                                x += chat.width;
                                unknown = -1;
                            } catch (Exception exception) {
    ...
    public void draw_glyph(int glyph, int x, int y, int width, int height, int color, boolean bool) {
        ...
        if (x + width > Rasterizer2D.bottomX) {
            int size = x + width - Rasterizer2D.bottomX;
            width -= size;
        ...
        if (width > 0 && height > 0) {
            createCharacterPixels(Rasterizer2D.pixels, glyph_pixels[glyph], color, src_pos, dst_pos, width, height, dst_width, src_width);
        }
    }
    ...
    public void draw_transparent_glyph(int glyph, int x, int y, int width, int height, int color, int alpha, boolean bool) {
        ...
        if (width > 0 && height > 0) {
            createTransparentCharacterPixels(Rasterizer2D.pixels, glyph_pixels[glyph], color, src_pos, dst_pos, width, height, dst_width, src_width, alpha);
        }
    }
