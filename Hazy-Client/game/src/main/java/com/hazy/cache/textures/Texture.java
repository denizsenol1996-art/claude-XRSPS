...
import com.hazy.collection.Linkable;
...
public class Texture extends Linkable implements RSTexture {
    ...
    Texture(int id, boolean transparent, int textureID, int animationSpeed, int animationDirection, int averageRGB) {
        this.isLoaded = false;
        this.id = id;
    ...
    boolean load(double brightness, int textureSize, Archive textureArchive) {

        for (int fileId : this.fileIds) {
            if (textureArchive.get(fileId + ".dat") == null) {
                return false;
            }
        }
    ...
    void animate(int var1) {
        boolean alwaysAnimateOnCPU = Model.alwaysAnimateCPU.contains(id) && Client.instance.isTexturesInventoryAnimated();
        if (Client.instance.isGpu() && !alwaysAnimateOnCPU) {
