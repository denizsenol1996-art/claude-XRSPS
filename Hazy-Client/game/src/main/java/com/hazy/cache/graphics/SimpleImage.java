...
public final class SimpleImage extends Rasterizer2D implements RSSpritePixels {
    ...
    public SimpleImage(byte[] data, int file) {
        try {
    ...
    public SimpleImage(byte data[], Component component) {
        try {
    ...
    public SimpleImage(String img, int width, int height) {
        try {
            ...
            x_offset = 0;
            y_offset = 0;
            myPixels = new int[this.width * this.height];
            PixelGrabber pixelgrabber = new PixelGrabber(image, 0, 0, this.width, this.height, myPixels, 0, this.width);
            pixelgrabber.grabPixels();
            image = null;
        } catch (Exception _ex) {
    ...
    public SimpleImage(Archive archive, String name, int i) {
        ...
        max_width = indexBuffer.readUShort();
        max_height = indexBuffer.readUShort();
        ...
        x_offset = indexBuffer.readUnsignedByte();
        y_offset = indexBuffer.readUnsignedByte();
        ...
        if (type == 1) {
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
                    myPixels[x + y * width] = raster[dataBuffer.readUnsignedByte()];
                }
            }
    ...
    public void drawHoverSprite(int x, int y, int offsetX, int offsetY, SimpleImage hover) {
        ...
        if (MouseHandler.mouseX >= offsetX + x && MouseHandler.mouseX <= offsetX + x + this.width
            && MouseHandler.mouseY >= offsetY + y && MouseHandler.mouseY <= offsetY + y + this.height) {
    ...
    public void draw24BitSprite(int x, int y) {
        ...
        int alpha = 256;
        x += this.x_offset;// offsetX
        y += this.y_offset;// offsetY
    ...
    public void init() {
        Rasterizer2D.initDrawingArea(height, width, myPixels,null);
    }
    ...
    public void method345() {
        ...
        for (int j = 0; j < height; j++) {
            System.arraycopy(myPixels, j * width, ai, j + y_offset * max_width + x_offset, width);
        }
    ...
    public void drawSprite(int i, int k, int color) {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                if (myPixels[x + y * width] != 0)
                    tempArray[(x + 1) + (y + 1) * tempWidth] = myPixels[x + y * width];
            }
        }
        ...
        try {
            ...
            for (j1 = 0; j1 < i; j1++) {
                ...
                for (k1 = -ai[j1]; k1 < 0; k1++) {
                    ...
                    int x2 = x1 + 1;
                    int y2 = y1 + 1;
                    int c1 = myPixels[x1 + y1 * width];
                    int c2 = myPixels[x2 + y1 * width];
                    int c3 = myPixels[x1 + y2 * width];
                    int c4 = myPixels[x2 + y2 * width];
                    int u1 = (k4 >> 8) - (x1 << 8);
                    int v1 = (l4 >> 8) - (y1 << 8);
                    int u2 = (x2 << 8) - (k4 >> 8);
                    int v2 = (y2 << 8) - (l4 >> 8);
    ...
    public void method353(int i, double d, int l1) {
        ...
        try {
            ...
            for (i = 0; i < k1; i++) {
                ...
                int j4 = j3;
                for (l1 = -k; l1 < 0; l1++) {
                    int k4 = myPixels[(i4 >> 16) + (j4 >> 16) * width];
                    if (k4 != 0)
    ...
    public void drawAdvancedSprite(int xPos, int yPos) {
        drawAdvancedSprite(xPos, yPos, 255);
    }
    ...
    public void cutR(int neww) {
        ...
        for (int k = 0; k < height; k++) {
            if (width * (k + 1) < myPixels.length)
                for (int dd = 0; dd < neww; dd++) {
                    if ((width * (k + 1)) - dd >= 0)
                        myPixels[(width * (k + 1)) - dd] = 0;
                }
            else
            ...
            for (int x = 0; x < width; x++) {
                ...
                if (outline == 0) {
                    if (x > 0 && myPixels[start - 1] != 0) {
                        outline = color;
                    } else if (y > 0 && myPixels[start - width] != 0) {
                        outline = color;
                    } else if (x < width - 1 && myPixels[start + 1] != 0) {
                        outline = color;
                    } else if (y < height - 1 && myPixels[start + width] != 0) {
                        outline = color;
                    }
                }
    ...
    public void shadow(int color) {
        ...
        for (int y = height - 1; y > 0; y--) {
            ...
            for (int x = width - 1; x > 0; x--) {
                if (myPixels[x + pos] == 0 && myPixels[x + pos - 1 - width] != 0) {
    ...
    public void cutL(int neww) {
        ...
        for (int k = 0; k < height; k++) {
            if ((k * (height * width)) + neww < myPixels.length)
                for (int dd = 0; dd < neww; dd++) {
                    if ((k * width) + dd >= 0)
                        myPixels[(k * width) + dd] = 0;
                }
    ...
    public Image convertToImage() {
        ...
        bufferedimage.setRGB(0, 0, width, height, myPixels, 0, width);
    ...
    public void drawShadedSprite(int xPos, int yPos, int shade) {
        ...
        if (!(spriteWidth <= 0 || spriteHeight <= 0)) {
            renderShadedARGBPixels(spriteWidth, spriteHeight, this.myPixels, Rasterizer2D.pixels, i1, shadeValue, j1, j2,
                i2);
        }
    ...
    public void draw_transparent(int x, int y, int alpha) {
        drawTransparentSprite(x,y,alpha);
    }
