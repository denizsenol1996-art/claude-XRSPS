...
public class TeleportWidget extends Widget {
    ...
    public static void handleTeleportTab(int index) {
        ...
        if (index == TELEPORT_CATEGORY_PVP) {
            ...
            newNames = new String[]{" Bandit Camp", "Chaos Temple", "Demonic Ruins", "East Dragons", "  Graveyard", "   Magebank", "   Rev Caves", "    The Gate", "West Dragons", "  Black Chins"};
            ...
            newNames = new String[]{"  Callisto", "   Cerberus", "   Chaos Fan", "  Corp Beast", "  Crazy Arch", "  Dagg Kings", "Demon Gorillas", "         GWD", "         KBD", "     Kraken", "     Shaman", "     Thermo", "   Venenatis", "      Vet'ion", "      Scorpia", "   Chaos Ele", "      Zulrah", "   Vorkath", "   World boss", "          KQ", "Giant Mole", "Alchy Hydra", "Barrelchest", "Corrupted Ne", "Raids Area", "Nex"};
            widget.totalChildren(newIDS.length * 2);
            //System.out.println("Widget length: " + widget.children.length);
            xPos = 35;
            yPos = 0;
    ...
    public static void updateTeleportsTab(boolean recent) {
        ...
        int xPos = 35;
        int yPos = 0;
        for (int i = 0; i < Client.instance.teleportSprites.length; i++) {
            if (i % 4 == 0 && i != 0) {
                xPos = 35;
                yPos += 68;
            } else {
            ...
            if (Client.instance.teleportCategoryIndex == 1) {
                Widget.addButtonWithMenu(TeleportWidget.STARTING_IMAGE_INDEX + i, Client.instance.teleportSprites[i], new String[]{"Teleport", null, "Remove from favourites", null, null});
                Widget.cache[STARTING_IMAGE_INDEX + i].width = 40;
                Widget.cache[STARTING_IMAGE_INDEX + i].height = 65;
                ...
                Widget.addButtonWithMenu(TeleportWidget.STARTING_IMAGE_INDEX + i, Client.instance.teleportSprites[i], new String[]{"Teleport", null, null, null, null});
                Widget.cache[STARTING_IMAGE_INDEX + i].width = 40;
                Widget.cache[STARTING_IMAGE_INDEX + i].height = 65;
            } else {
                Widget.addButtonWithMenu(TeleportWidget.STARTING_IMAGE_INDEX + i, Client.instance.teleportSprites[i], new String[]{"Teleport", "Add to favourites", null, null, null});
                Widget.cache[STARTING_IMAGE_INDEX + i].width = 40;
                Widget.cache[STARTING_IMAGE_INDEX + i].height = 65;
            }
        ...
        int y1 = 50;
        AdvancedFont[] fonts = {Client.adv_font_small, Client.adv_font_regular, Client.adv_font_bold, Client.adv_font_fancy};
        for (int i = 0; i < Client.instance.teleportNames.length; i++) {
            if (i % 4 == 0 && i != 0) {
                x1 = 38;
                y1 += 68;
            } else {
            ...
            Widget.addText(29195 + i, Client.instance.teleportNames[i], fonts, 1, 0xffffff, false, false);
            widget.child(Client.instance.teleportSprites.length + i, 29195 + i, x1, y1);
        }
        ...
        if (Client.instance.teleportSprites != null && Client.instance.teleportSprites.length > 12) {
            int offset = 69;
            int columns = Client.instance.teleportSprites.length / 4;
