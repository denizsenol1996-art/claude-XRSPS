...
@Slf4j
public class NpcSpawnInWild extends Task {
     ...
     */
    public NpcSpawnInWild() {
        super("NpcSpawnInWild", INTERVAL);
    }
    ...
    final int[] NPCS =
        {
            MENDING_REVENANT_KNIGHT,
            MENDING_REVENANT_ORK
        };
    ...
    public void execute() {
        ...
        if (!loaded) {
            for (final int region : REGIONS) {
                ...
                while (count.get() < 64 && maxIterations-- > 0) {
                    ...
                    if (RegionManager.zarosBlock(tile) || tiles.contains(tile) || ObjectManager.objWithTypeExists(10, tile)
        ...
        for (Tile tile : tiles) {
            ...
            int id = Utils.randomElement(NPCS);
            Npc revenants = new Npc(id, tile);
            revenants.walkRadius(5);
            revenants.spawn(false);
            spawned++;
        }
    }
