...
public class DwarfCannon extends OwnedObject {
    ...
    public void fill() {
        if (getOwner().isPlayer()) {
            ...
            MAX_AMMO = player.getMemberRights().isDiamondOrGreater(player) ? 50 : MAX_AMMO;
        }
            ...
            if (!MultiwayCombat.includes(getOwner()) && Objects.nonNull(getOwner().getCombat().getTarget())) {
                ...
                target =
                    Arrays.stream(getOwner().closeNpcs(48))
                        ...
                        .filter(npc -> ProjectileRoute.allow(getCorrectedTile(tile()).getX(), getCorrectedTile(tile()).getY(), getCorrectedTile(tile()).getZ(), 1, npc.tile().getX(), npc.tile().getY(), npc.getSize()))
        ...
        target.ifPresent(
            npc -> {
                getOwnerOpt().ifPresent(owner -> {
                    Projectile cannonBall = new Projectile(getCorrectedTile(tile()), npc.tile(), 53, npc.getProjectileLockonIndex(), 85, 35, 20, 20, 0, 0, 64);
                    cannonBall.sendProjectile();
                    var tileDist = owner.tile().transform(3, 3, 0).distance(npc.tile());
                    var delay = Math.max(1, (20 + (tileDist * 12)) / 30);
                    var hit = new Hit(owner, npc, Utils.random(1, owner.getCombat().maximumRangedHit(true)), delay, CombatType.RANGED);
                    hit.submit();
                    hit.postDamage(h1 -> {
    ...
    public boolean handleAreaRestriction() {
        ...
        if (player.tile().memberCave()) {
            player.message("You can't place a cannon in dcave.");
            return false;
