package com.twisted.game.world.route;
...
import com.twisted.game.world.route.types.RouteAbsolute;
import com.twisted.game.world.route.types.RouteEntity;
import com.twisted.game.world.route.types.RouteObject;
import com.twisted.game.world.route.types.RouteRelative;
import com.twisted.net.packet.incoming_packets.ObjectClickOnePacketListener;
...
public class RouteFinder {
    ...
    public void route(RouteType route, boolean message, boolean ignoreFreeze) {
        ...
        MovementQueue movement = entity.getMovementQueue();
    ...
    public void routeGroundItem(GroundItem groundItem, Consumer<Integer> successConsumer) {
        ...
        Chain.bound(entity).repeatingTask(1, t -> { // problem is this isnt a chain, oops
            if (!entity.getMovementQueue().isAtDestination() || !route.finished(entity.getCentrePosition()))
                return;
            if (route.reachable) {
            ...
            entity.face(groundItem.getTile().x, groundItem.getTile().y);
            if (entity.isPlayer())
                entity.getAsPlayer().getMovementQueue().outOfReach();
            t.stop();
        });
    }
    ...
    public void routeObject(GameObject gameObject, Runnable successAction) {
        ...
        Chain.bound(entity).repeatingTask(1, event -> {
            if (!entity.getMovementQueue().isAtDestination()) {
    ...
    public boolean allowStep(int stepX, int stepY) { //issue here for verzik pathing
        ...
        if (entity.getMovementQueue().stepType == StepType.NORMAL) {
            if (!entity.getMovementQueue().canMove(!entity.isNpc() && entity.getAsPlayer().getMovementQueue().movementPacketThisCycle())) {
                return false;
            }
            if (entity.isNpc()&& Tile.isOccupied(entity, stepX, stepY)) {
                entity.getMovementQueue().reset();
                return false;
            }
