package com.twisted.game.world.position;

import com.twisted.game.content.areas.riskzone.RiskFightArea;
...
import static com.twisted.game.world.route.RouteFinder.*;
...
public class Tile implements Cloneable {
    ...
    public static final Area RAID = new Area(3212, 5119, 3367, 5763, -1);
    public static final Area COS = new Area(3130, 4545, 3200, 4665, -1);//updatet3
    public static final Area DITCH = new Area(3041, 3515, 3122, 3527, -1);
    public static final Area MB = new Area(3090, 3917, 3121, 3952, -1);//MB AREA

    public static final Area CRAZY1 = new Area(2468, 2764, 2475, 2772, 0);//updatet3
    public static final Area CRAZY2 = new Area(2446, 2764, 2453, 2772, 0);//updatet3

    public static final Area DRAGON1 = new Area(2468, 2780, 2475, 2792, 0);//updatet3
    public static final Area DRAGON2 = new Area(2446, 2779, 2453, 2791, 0);//updatet3


    public static final Area GRIM1 = new Area(2468, 2798, 2475, 2806, 0);//updatet3
    public static final Area GRIM2 = new Area(2446, 2798, 2453, 2806, 0);//updatet3

    public static final Area REVS = new Area(2458, 2800, 2463, 2807, 0);//updatet3


    public boolean homeRegion() {
        return inArea(EDGEVILE_HOME_AREA) || region() == 7991 || region() == 7992 || region() == 8247;//I CAN DO QUICK RESET OR JUST UPDATE?
    }

    public static boolean inSerenRoom(Player player) {//updateseren
        return player.tile().inArea(3280, 3988, 3306, 4013);
    }

    public static boolean inDzoneThree(Player player) {//updateseren
        return player.tile().inArea(2890, 2773, 2906, 2780);
    }
    ...
    public boolean allowObjectPlacement() {
        if (ObjectManager.objWithTypeExists(10, this) || ObjectManager.objWithTypeExists(11, this)) {
            return false;
        }
        return true;
    }
    ...
    public boolean isViewableFrom(Tile other) {
        if (this.getLevel() != other.getLevel())
            return false;
        int[] delta = new int[2];
        delta(this, other, delta);
        return delta[0] <= 14 && delta[0] >= -15 && delta[1] <= 14 && delta[1] >= -15;
    }

    public static void delta(Tile a, Tile b, int[] result) {
        result[0] = b.getX() - a.getX();
        result[1] = b.getY() - a.getY();
    }
    ...
    public Set<Tile> expandedBounds(int radius, Predicate<Tile> filter) {
        ...
        for (int x = -radius; x <= radius; x++) {
            Tile tile1 = relative(x, -radius);
            if (filter.test(tile1)) {
                list.add(tile1);
            }

            Tile tile2 = relative(x, +radius);
            if (filter.test(tile2)) {
                list.add(tile2);
            }
        }

        for (int y = -radius; y <= radius; y++) {
            Tile tile3 = relative(+radius, y);
            if (filter.test(tile3)) {
                list.add(tile3);
            }

            Tile tile4 = relative(-radius, y);
            if (filter.test(tile4)) {
                list.add(tile4);
            }
        }
    ...
    public static boolean inRaid1(Player player) {//upos
        return player.tile().region() == 13137 || player.tile().region() == 13138 || player.tile().region() == 13139
            || player.tile().region() == 12889;
    }

    public static boolean inRaid2(Player player) {//upos
        return player.tile().region() == 12615 || player.tile().region() == 12616;

    }

    public static boolean inTob(Player player) {
        return player.tile().region() == 12613 || player.tile().region() == 13125 || player.tile().region() == 13122
            || player.tile().region() == 13123 || player.tile().region() == 12612 || player.tile().region() == 12611
            || player.tile().region() == 12867;
    }
    ...
    public boolean equalsIgnoreHeight(Object obj) {
        if (!(obj instanceof Tile))
            return false;
        Tile o = (Tile) obj;
        return o.x == x && o.y == y;
