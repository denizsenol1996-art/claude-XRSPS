package com.twisted.game.world.items.container.looting_bag;
...
import static com.twisted.game.world.entity.AttributeKey.LOOTING_BAG_UPGRADE;
import static com.twisted.util.ItemIdentifiers.LOOTING_BAG;
import static com.twisted.util.ItemIdentifiers.LOOTING_BAG_22586;
...
public class LootingBag extends ItemContainer {
    ...
    public void open() {
        if (player.getParticipatingTournament() != null) {
            player.message("You cannot use your looting pouch inside the tournament.");
            return;
        }
        if (player.getRaids() != null) {
            player.message("You cannot use your looting pouch inside of a raid.");
            return;
        }
    ...
    public Item[] dropItemsOnDeath() {
        if (!player.inventory().contains(11941) && !player.inventory().contains(22586))
            return null;
        if(!player.inventory().contains(30099) && !player.inventory().contains(30098))
            return null;
    ...
    public void openAndCloseBag(int bag_id) {
        ...
        switch (bag_id) {
            case 30099 -> {
                player.inventory().replace(30099, 30098, true);
                player.message("You close your looting bag I.");
            }
            case 30098 -> {
                player.inventory().replace(30098, 30099, true);
                player.message("You open your looting bag I ready to fill it.");
            }
            case 11941 -> {
    ...
    public void setSettings() {
        ...
        if (!player.inventory().contains(11941) && !player.getInventory().contains(30099))
            return;
    ...
    public boolean deposit(Item item, int amount, GroundItem groundItem) {
        ...
        var claimedUpgrade = player.<Boolean>getAttribOr(LOOTING_BAG_UPGRADE, false);
        ...
        if (item.getId() == CustomItemIdentifiers.ESCAPE_KEY) {
            player.message("You cannot add the Escape key to your looting bag.");
            return false;
        }
        ...
        if (item.getId() == 30099 || item.getId() == 30098) {
            player.message("You may be surprised to learn that bagception is not permitted.");
            return false;
        }

        // Is the player inside the wilderness?s
        if (!WildernessArea.inWilderness(player.tile()) && !player.getMemberRights().isSaphireOrGreater(player)) {
            player.message("You can't put items in the looting bag unless you're in the Wilderness.");
            return false;
        }

        // Is the item trade-able?//IBAG
        if (!item.rawtradable() && !player.getInventory().contains(30099)&& !player.getInventory().contains(30098)) {
            player.message("Only tradeable items can be put in the bag or use looting bag (I).");
            return false;
        }
    ...
    public boolean itemOnItem(Item used, Item with) {
        ...
        if (!(used.getId() == LOOTING_BAG || with.getId() == LOOTING_BAG|| used.getId() == LOOTING_BAG_22586 || with.getId() == LOOTING_BAG_22586 || used.getId() == 30099 || with.getId() == 30099 || used.getId() == 30098 || with.getId() == 30098 )) {
            return false;
        }
        if (used.getId() == 30099) {
    ...
    public static int OPEN_LOOTING_BAG_I = 30098;
    ...
    public boolean lootbagOpen() {
        ...
        return (player.inventory().contains(OPEN_LOOTING_BAG_I)) || player.inventory().contains(OPEN_LOOTING_BAG_I) || (player.inventory().contains(OPEN_LOOTING_BAG)) || player.inventory().contains(OPEN_LOOTING_BAG);
