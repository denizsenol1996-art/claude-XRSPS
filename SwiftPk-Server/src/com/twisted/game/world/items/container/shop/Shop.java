package com.twisted.game.world.items.container.shop;
...
import com.twisted.game.world.items.container.shop.currency.CurrencyType;
...
import static com.twisted.util.CustomItemIdentifiers.BIG_CHEST;
import static com.twisted.util.CustomItemIdentifiers.BLOOD_MONEY_CASKET;
...
public abstract class Shop {
    ...
    public void purchase(Player player, Item item, int slot) {
        ...
        if (player.getRaids() != null) {
            player.message("You cannot use the shop inside of a raid.");
            return;
        }
        ...
        if (!player.inventory().hasCapacityFor(item) && !item.noteable()) {
        ...
        Utils.sendDiscordInfoLog(player, player.getUsername() + " has bought " + item.unnote().name() + " from a shop for " + Utils.formatNumber((long) item.getAmount() * value) + " " + currencyType.currency.toString(), "shops");
    ...
    protected final void sell(Player player, Item item, int slot) {
        ...
        if (item.getId() == BLOOD_MONEY_CASKET || item.getId() == BIG_CHEST || item.getId() == COINS_995 || item.getId() == PLATINUM_TOKEN || item.getId() == BLOOD_MONEY || item.getId() == CustomItemIdentifiers.HAZY_COINS) {
...
/*        if (item.unnote().definition(World.getWorld()).pvpAllowed) {
        ...
        int sellValue;
        sellValue = item.getId() == 619 ? 1 : item.getId() == 12934 ? 2 : item.getSellValue();//its up to data now so his fixes?

        //seems like he made it 2 wh at was ibefore?//you couldnt sell it before so thats why i wanted it 2/ is it 12934? no clue lets see l0l

        //idk what he was trying to do // like what he fixed or? ye
        ...
        if (sellValue > 0) {
            ...
            Utils.sendDiscordInfoLog(player, player.getUsername() + " has sold " + item.unnote().name() + " for " + Utils.formatNumber((long) item.getAmount() * sellValue) + " " + currencyType.currency.toString() + ".", "shops");
        }
...
/*        if (item.unnote().definition(World.getWorld()).pvpAllowed) {
        ...
        int value = item.getId() == 619 ? 1 : item.getId() == 12934 ? 2 : item.getSellValue();
