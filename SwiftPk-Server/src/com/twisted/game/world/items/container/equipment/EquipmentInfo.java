...
import static com.twisted.util.ItemIdentifiers.OSMUMTENS_FANG_OR;
...
public class EquipmentInfo {
    ...
    private final Int2IntMap weaponSpeeds = new Int2IntOpenHashMap();
    ...
    public static EquipmentBonuses criticalBonuses(Player player) {
        EquipmentInfo info = World.getWorld().equipmentInfo();
        EquipmentBonuses bonuses = player.getBonuses();
    ...
    private void loadEquipmentDefinitions(File file) {
        try {
            equipmentDefinitions = gson.fromJson(new FileReader(file), new TypeToken<Int2ObjectOpenHashMap<EquipmentDefinition>>() {}.getType());
    ...
    public static void loadBonuses(File file) {
        try {
            bonuses = gson.fromJson(new FileReader(file), new TypeToken<Int2ObjectOpenHashMap<EquipmentBonuses>>() {}.getType());
    ...
    public static int attackAnimationFor(Player player, int weapon) {
        ...
        if (weapon != 0) {
            switch (weapon) {
                ...
                case MENDING_WAND://BLOODREVSUPDATE
                case _3RD_AGE_WAND:
                    return 414;
