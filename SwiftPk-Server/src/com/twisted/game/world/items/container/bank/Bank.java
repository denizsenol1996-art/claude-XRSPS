package com.twisted.game.world.items.container.bank;
...
public class Bank extends ItemContainer {
    ...
    public void deposit(int slot, int amount, ItemContainer fromIc) {
        ...
        if (player.inActiveTournament() || player.isInTournamentLobby()) return;
        ...
        if (amount > 0) fromIc.remove(item.getId(), amount);
        setFiringEvents(true);
        refresh();
    }
    ...
    public boolean buttonAction(int button) {
        if (player.getParticipatingTournament() != null) return false;
        if (player.getRaids() != null) return false;
        if (!player.getInterfaceManager().isInterfaceOpen(InterfaceConstants.BANK_WIDGET) && !player.getInterfaceManager().isInterfaceOpen(34000)) return false;
