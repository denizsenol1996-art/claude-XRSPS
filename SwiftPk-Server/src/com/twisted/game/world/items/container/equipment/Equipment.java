...
import com.twisted.game.content.items.equipment.max_cape.MaxCape;
...
import com.twisted.game.world.items.ItemWeight;
import com.twisted.game.world.items.container.ItemContainer;
import com.twisted.game.world.items.container.ItemContainerAdapter;
...
public final class Equipment extends ItemContainer {
    ...
    public static boolean wearingAvasEffect(Player player) {
        Item cape = player.getEquipment().get(EquipSlot.CAPE);
        return cape != null && player.getEquipment().containsAny(10498, 10499, 13337, 9756, 9757, RANGING_CAPE, RANGING_CAPET, 22109, 21898, MASORI_ASSEMBLER, MASORI_ASSEMBLER_L, MASORI_ASSEMBLER_MAX_CAPE, MASORI_ASSEMBLER_MAX_CAPE_L, 28902, 28906, 24855);
    }
    ...
    public int hpIncrease() {
        ...
        /*for (int index = 0; index < getItems().length; index++) {
            ...
            if (index == EquipSlot.HEAD) {
                if (BLOOD_TORVA_HELM == id)
                    hpIncrease += 6;
            } else if (index == EquipSlot.BODY) {
                if (BLOOD_TORVA_PLATEBODY == id)
                    hpIncrease += 20;
            } else if (index == EquipSlot.LEGS) {
                if (BLOOD_TORVA_PLATELEGS == id)
                    hpIncrease += 13;
            }
        }*/
    ...
    private final int[] SLAYER_HELMETS = new int[]{RED_SLAYER_HELMET_I,
        BLACK_SLAYER_HELMET_I, GREEN_SLAYER_HELMET_I, TURQUOISE_SLAYER_HELMET_I, 30326, HYDRA_SLAYER_HELMET_I, SLAYER_HELMET, SLAYER_HELMET_I, RED_SLAYER_HELMET, GREEN_SLAYER_HELMET, BLACK_SLAYER_HELMET, PURPLE_SLAYER_HELMET, PURPLE_SLAYER_HELMET_I, TURQUOISE_SLAYER_HELMET, TWISTED_SLAYER_HELMET, TWISTED_SLAYER_HELMET_I, 25913, 25902};
    ...
    public boolean equip(int inventoryIndex) {
        ...
        boolean[] needsreq = Prestige.hasPrestigeRequirement(player, equip);
        ...
        if (player.getEquipment().hasAt(EquipSlot.WEAPON, TRIDENT_OF_THE_SEAS) || player.getEquipment().hasAt(EquipSlot.WEAPON, TRIDENT_OF_THE_SEAS_FULL)) {
        ...
        } else if (player.getEquipment().hasAt(EquipSlot.WEAPON, SANGUINESTI_STAFF) || player.getEquipment().hasAt(EquipSlot.WEAPON, CustomItemIdentifiers.HOLY_SANGUINESTI_STAFF)) {
            ...
            Autocasting.setAutocast(player, CombatSpells.TUMEKENS_SPELL.getSpell());
        ...
        } else if (player.getEquipment().hasAt(EquipSlot.WEAPON, ELDER_WAND) || player.getEquipment().hasAt(EquipSlot.WEAPON, ELDER_WAND_RAIDS)) {
            Autocasting.setAutocast(player, CombatSpells.CRUCIATUS_CURSE.getSpell());
        } else if (player.getEquipment().hasAt(EquipSlot.WEAPON, THAMMARONS_STAFF_C)) {
            Autocasting.setAutocast(player, CombatSpells.THAMMARON_SCEPTRE.getSpell());
        } else {
            player.getCombat().setCastSpell(null);
            Autocasting.setAutocast(player, null);
        }
    ...
    public Item getSlotForItem(int slot) {
        return get(slot);
    }

    public Item getLegs() {
        return get(EquipSlot.LEGS);
    }

    public Item getBoots() {
        return get(EquipSlot.FEET);
    }

    public Item getGloves() {
        return get(EquipSlot.HANDS  );
    }
    ...
    public static boolean venomHelm(Mob entity) {
        Player player = (Player) entity;
        Item helm = player.getEquipment().get(EquipSlot.HEAD);
        if (helm == null)
            return false;
        return helm.getId() == 12931 || helm.getId() == 13197 || helm.getId() == 13199 || player.getEquipment().containsAny(SERPENTINE_HELM, MAGMA_HELM, TANZANITE_HELM);
    }
