package com.twisted.game.world.items.ground;
...
public final class GroundItemHandler {
    ...
    public static void pickup(Player player, int id, Tile tile, boolean telekenesis) {
        ...
        } else if (player.ironMode().isIronman()) {
            if (different_owner && !groundItemPked) { // Owner different? It could be pked!
        ...
        Chain.noCtx().runFn(1, () -> {
            ...
            if (!player.tile().isWithinDistance(groundItem.getTile(), 1) && !telekenesis) {
                System.out.println("Player is not within distance of the item.");
                return;
            }
            ...
            Utils.sendDiscordInfoLog(player, "Player " + player.getUsername() + " picked up item " + item.getAmount() + "x " + item.unnote().name() + " (id " + item.getId() + ") at X: "+groundItem.getTile().x+" Y: "+groundItem.getTile().y, "pickups");player.getInventory().refresh();
