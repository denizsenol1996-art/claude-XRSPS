    ...
    public static void loadMapFiles(int x, int y, boolean force) {
        try {
            ...
            if (r.getObjectFile() != -1)
                oFileData = CompressionUtil.gunzip(FileUtil.readFile(GameServer.properties().clippingDirectory + "maps/" + r.getObjectFile() + ".gz"));
            if (r.getTerrainFile() != -1)
                gFileData = CompressionUtil.gunzip(FileUtil.readFile(GameServer.properties().clippingDirectory + "maps/" + r.getTerrainFile() + ".gz"));
