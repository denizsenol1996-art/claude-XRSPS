package com.twisted.game.world.entity.combat.method.impl.npcs.bosses.custom;
...
public class BrutalLavaDragon2 extends CommonCombatMethod {
    ...
    public boolean prepareAttack(Mob mob, Mob target) {
            ...
            if (World.getWorld().rollDie(10, 1) && !WildernessArea.inDiamondZone(target.tile())) {
    ...
    private void magicAttack(Npc npc, Mob target) {
        npc.face(null); // Stop facing the target
            if (target.tile().inArea(2445,2779,2453,2791) || target.tile().inArea(2468,2779,2476,2791))  {
                if (target != null && target.tile().inSqRadius(target.tile(), 12)) {
                    ...
                    Projectile DRAGONFIRE_PROJ_FLYING = new Projectile(mob, target, 54, 50, 12 * tileDist, 150, 32, 0);
                    Projectile FIRE_PROJ_FLYING = new Projectile(mob, target, 1465, 51, 12 * tileDist, 160, 31, 0);
