    ...
    public enum MagicSpells {
        ...
        TELEPORT_TO_TARGET_NORMAL(new Spell() {
            ...
            public boolean canCast(Player player, Mob target, boolean delete) {
                ...
                if (targetFor.isPresent()) {
                    ...
                    if(targetFor.get().tile().memberCave() && !player.getMemberRights().isSaphireOrGreater(player)) {
                    ...
                    if(teleport_delay.active()){
                        int seconds = teleport_delay.secondsRemaining();
                        player.message("You must wait another " + (seconds == 1 ? "second" : "" + seconds + " seconds") + " before teleport again to your target.");
                        return false;
                    }
                } else {
        ...
        TELEPORT_TO_TARGET_ANCIENT(new Spell() {
            ...
            public boolean canCast(Player player, Mob target, boolean delete) {
                ...
                if (targetFor.isPresent()) {
                    ...
                    if(targetFor.get().tile().memberCave() && !player.getMemberRights().isRubyOrGreater(player)) {
                        player.message("Your target is currently in the member cave. You cannot teleport there because you are not a member.");
                        return false;
                    }
        ...
        CHARGE(new Spell() {
            ...
            public void startCast(Mob cast, Mob castOn) {
                ...
                if (player != null) {
                    player.getTimerRepository().register(TimerKey.CHARGE_SPELL, 200);
        ...
        VENGEANCE(new Spell() {
            ...
            public void startCast(Mob cast, Mob castOn) {
                ...
                if (player != null) {
                    if (!player.locked()) {
                        if (!player.getTimerRepository().has(TimerKey.VENGEANCE_COOLDOWN)) {
