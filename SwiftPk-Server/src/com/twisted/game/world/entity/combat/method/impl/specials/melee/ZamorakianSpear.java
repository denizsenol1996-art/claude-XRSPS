...
public class ZamorakianSpear extends CommonCombatMethod {
    ...
    @Override
    public boolean prepareAttack(Mob attacker, Mob victim) {
        attacker.animate(1064);
        attacker.graphic(253, 100, 0);

        attacker.face(victim.tile());
        attacker.getTimerRepository().register(TimerKey.COMBAT_ATTACK, 2);
        ...
        if(attacker.isPlayer() && victim.isPlayer()) {
            ...
            victim.getTimerRepository().extendOrRegister(TimerKey.COMBAT_LOGOUT, 16);
            ...
            if (!legal) {
                ...
                if (!legal) {
                    return false; // No valid move to go!
                }
            }
