...
public class Dzone4Command implements Command {
    @Override
    public void execute(Player player, String command, String[] parts) {
        double totalAmountPaid = player.getAttribOr(AttributeKey.TOTAL_PAYMENT_AMOUNT, 0D);

        if (totalAmountPaid < 1500 && !player.getPlayerRights().isAdminOrGreater(player)) {
            player.message("@red@You need 1500 total donated to access this area.");
            return;
        }
        ...
        if (Teleports.canTeleport(player, true, TeleportType.GENERIC)) {
            Teleports.basicTeleport(player, new Tile(2461, 2708));
        }
