...
import com.twisted.game.content.achievements.AchievementWidget;
import com.twisted.game.content.areas.wilderness.content.EloRating;
...
public final class QuestTab {
    ...
    public enum InfoTab {
        ...
        TORYPOINTS(12673) {
            ...
            public String fetchLineData(Player player) {//this tournament point in quest tab perfect!
                ...
                return "Tournament Points: <col=ffffff>" + Utils.formatNumber(turypoints);
            }
        },
        REFERRALS(12674) {
            @Override
        ...
        PLAYERS_ONLINE(12700) {
            ...
            public String fetchLineData(Player player) {
                ...
                if (player.getPlayerRights().isOwner(player)) {
                    int realCount = (int) (count / 1.35);
                    return "Players Online: <col=ffffff>R:" + count + " (A: " + (realCount - count) + ")";
                }
    ...
    public static boolean onButton(Player player, int button) {
        switch (button) {
            case 12661 -> {
                player.getPacketSender().sendURL("http://hazyrealm.com/");
                player.message("Opening http://hazyrealm.com/ in your web browser...");
            }

            case 12662 -> {
                player.getPacketSender().sendURL("https://discord.gg/hN4HDwgq7f");
