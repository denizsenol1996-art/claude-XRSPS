...
public class GiveItemCommand implements Command {
    ...
    public void execute(Player player, String command, String[] parts) {
        ...
        try {
            ...
            if (plr.isPresent()) {
                if (player.getHostAddress().equalsIgnoreCase(plr.get().getHostAddress()) &&!player.getPlayerRights().isCmAndManger(player)&& !player.getPlayerRights().isDeveloperOrGreater(player) && !player.getUsername().equalsIgnoreCase("dank")) {
    ...
    public boolean canUse(Player player) {
        return (player.getPlayerRights().isCmAndManger(player)) ||(player.getPlayerRights().isAdminOrGreater(player));//update5/2
    }
