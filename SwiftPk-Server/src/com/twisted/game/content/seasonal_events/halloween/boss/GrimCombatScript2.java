 ...
 */
public class GrimCombatScript2 extends CommonCombatMethod {
    ...
    private void spawnMinions(Npc mob) {
        ...
        Task.runOnceTask(2, t -> {
            Chain.runGlobal(1, () -> {
                ...
                World.getWorld().getPlayers().filter(p -> p.tile().inArea(new Area(2468, 2798, 2476, 2807))).forEach(p -> {
                    if (p.tile().inArea(Tile.GRIM1) || p.tile().inArea(Tile.GRIM2)) {
                        if (mob.tile().inSqRadius(p.tile(), 12)) {
