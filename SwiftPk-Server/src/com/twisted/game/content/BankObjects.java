...
public class BankObjects extends PacketInteraction {
    ...
    public boolean handleItemOnObject(Player player, Item item, GameObject obj) {
        final ObjectDefinition definition = obj.definition();
        if (definition != null) {
            final String name = definition.name;
            if (name != null) {
                if (name.equalsIgnoreCase("bank booth")
                    || name.equalsIgnoreCase("grand exchange booth")
                    || name.equalsIgnoreCase("bank chest")) {
                    final int itemid = player.getAttribOr(AttributeKey.ITEM_ID, -1);
