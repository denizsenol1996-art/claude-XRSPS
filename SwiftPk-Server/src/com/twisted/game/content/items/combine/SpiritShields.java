...
public class SpiritShields extends PacketInteraction {
    ...
    public boolean handleItemOnObject(Player player, Item item, GameObject object) {
        if (object.definition().name.equalsIgnoreCase("anvil")) {
            if (item.getId() == CustomItemIdentifiers.DIVINE_SIGIL) {
                ...
                Chain.bound(player).name("SpiritShieldsAnvilTask").runFn(6, () -> player.animate(898)).then(6, () -> {
                    if (player.inventory().containsAll(BLESSED_SPIRIT_SHIELD, CustomItemIdentifiers.DIVINE_SIGIL)) {
                        player.inventory().remove(new Item(BLESSED_SPIRIT_SHIELD),true);
                        player.inventory().remove(new Item(CustomItemIdentifiers.DIVINE_SIGIL),true);
                        player.inventory().add(new Item(CustomItemIdentifiers.DIVINE_SPIRIT_SHIELD),true);
                    }
                    player.getDialogueManager().start(new Dialogue() {
                        @Override
                        protected void start(Object... parameters) {
                            send(DialogueType.ITEM_STATEMENT, new Item(CustomItemIdentifiers.DIVINE_SPIRIT_SHIELD), "", "You successfully combine the Divine sigil with the shield.");
                            setPhase(0);
                        }
