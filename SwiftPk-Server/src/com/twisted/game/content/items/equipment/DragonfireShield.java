...
import static com.twisted.util.ItemIdentifiers.DRAGONFIRE_WARD;
...
public class DragonfireShield extends PacketInteraction {
    ...
    private void executeSpecialAttack(Player player, DragonfireShieldType type) {
        if ((GameServer.properties().production || !player.getPlayerRights().isDeveloperOrGreater(player)) && player.getTimerRepository().has(TimerKey.DRAGONFIRE_SPECIAL)) {
            ...
            if (target != player && !player.dead() && CombatFactory.inCombat(player) && inDistance) {
                ...
                player.getTimerRepository().extendOrRegister(TimerKey.COMBAT_ATTACK, 4);
                player.getTimerRepository().register(TimerKey.DRAGONFIRE_SPECIAL, 200);
