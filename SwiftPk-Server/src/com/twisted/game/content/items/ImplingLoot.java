...
import static com.twisted.util.ItemIdentifiers.IMPLING_JAR;
...
public class ImplingLoot extends PacketInteraction {
    ...
    public enum ImplingJar {
        ...
        public static ImplingJar forJarId(int id) {
            for (ImplingJar implingJar : values()) {
                if (implingJar.itemId == id) {
                    return implingJar;
                }
            }
            return null;
        ...
        public static void loot(Player player, Item item, ImplingJar implingJar) {
            // OSRS demands 2 free slots.
            if (player.inventory().getFreeSlots() < 2) {
                player.message("You'll need to clear some space in your pack before looting the jar.");
                return;
            }
            ...
            if (World.getWorld().rollDie(10, 1)) {
                player.message("You break the jar as you try and open it. You throw the shattered remains away.");
            } else {
                //Add empty impling jar
                player.inventory().add(new Item(IMPLING_JAR), true);
    ...
    public boolean handleItemInteraction(Player player, Item item, int option) {
        for (ImplingJar implingJar : ImplingJar.values()) {
            if (item.getId() == implingJar.itemId) {
                ImplingJar.loot(player, item, implingJar);
                return true;
            }
        }
        return false;
