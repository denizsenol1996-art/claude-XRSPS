package com.twisted.game.content.items_kept_on_death;
...
public class ItemsKeptOnDeath {
    ...
    /**
     * attempts should be made to match and consolidate the logic into {@link com.twisted.test.unit.PlayerDeathDropResult}
     * @param player
     */
    public static void calculateItems(Player player) {
        ...
        player.inventory().forEach(item -> {//ibag
            if (!item.matchesId(RUNE_POUCH)  && !item.matchesId(30099) && !item.matchesId(30098) && !item.matchesId(LOOTING_BAG) && !item.matchesId(LOOTING_BAG_22586)) {
                tempToDrop.add(item);
            } else {
     ...
     * mimics {@link com.twisted.game.task.impl.PlayerDeathTask#dropItems(Player, Mob)} {@code toDrop.foreach() block}
     */
