...
import static com.twisted.util.NpcIdentifiers.LISA;
import static com.twisted.util.ObjectIdentifiers.EXIT_PORTAL_27096;
import static java.lang.String.format;

/**
 * A tournament system. Configured via /data/def/arcutus/tournaments.conf file
 * <br>
 * You can start tournys via commands ::settornhours 05.50,05.51 etc {@link com.twisted.game.world.entity.mob.player.commands.impl.dev.SetTornLobbyTime}
 * <br>
 * ::settornlobbytime 5 {@link com.twisted.game.world.entity.mob.player.commands.impl.dev.SetTornLobbyTime}
 * <br>
...
public class TournamentManager extends PacketInteraction {
        ...
        if (player.inActiveTournament()) {
            ...
            if (torn.fighters.contains(player)) {
                ...
                if (torn.winner == player) {
                    ...
                    Utils.sendDiscordInfoLog(player, "Player " + player.getUsername() + " received a tournament reward: " + torn.reward.getAmount() + " x " + torn.reward.name() + ".", "tournaments");
                    if (torn.reward != null) {
    ...
    public static boolean handleDeath(Player player) {
        if (!player.inActiveTournament()) return false;
    ...
    public static boolean canAttack(Mob attacker, Mob target) {
        if (attacker.isPlayer() && attacker.getAsPlayer().getParticipatingTournament() != null) {
            if (target.isPlayer() && target.getAsPlayer().getTournamentOpponent() == attacker && target.getTimerRepository().has(TimerKey.TOURNAMENT_FIGHT_IMMUNE)) {
            ...
            if ((System.currentTimeMillis() - next.lobbyOpenTimeMs) / 1000 >= TournamentManager.getSettings().lobbyTime) {
                ...
                if (next.inLobby.size() < next.minimumParticipants) {
                    ...
                    for (Iterator<Player> iterator = next.inLobby.iterator(); iterator.hasNext(); ) {
                        ...
                        Utils.sendDiscordInfoLog(player,"Player " + player.getUsername() + " been teleported to the combat area!.", "tournaments");
