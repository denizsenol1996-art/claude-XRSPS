package com.twisted.game.content.trade;
...
import com.twisted.game.world.items.container.ItemContainer.StackPolicy;
...
import static com.twisted.util.CustomItemIdentifiers.ESCAPE_KEY;
import static com.twisted.util.CustomItemIdentifiers.WILDERNESS_KEY;
...
public class Trading {
    ...
    public void requestTrade(Player otherPlayer) {
        ...
        if (player.getTimerRepository().has(TimerKey.SPECIAL_TELEBLOCK)) {
            player.teleblockMessage();
            return;
        }

        if (player.inventory().contains(WILDERNESS_KEY)) {//up02
            player.message(Color.RED.wrap("You can't send trade requests when holding the Wilderness key."));
            return;
        }

        if (player.inventory().contains(ESCAPE_KEY)) {//up02
      ...
      /*  if (player.ironMode() != IronMode.NONE && (otherPlayer == null || !(otherPlayer.getPlayerRights().isDeveloperOrGreater(otherPlayer) || otherPlayer.ironMode().isGroupIronman()))) {
        ...
        if ((player.ironMode() != IronMode.NONE && !player.mode().isDarklord()) || (otherPlayer != null && otherPlayer.ironMode() != IronMode.NONE && !otherPlayer.mode().isDarklord())) {
            player.message("You are an Iron Man. You stand alone.");
            return;
        }


        if (otherPlayer != null && player.mode().isDarklord() && !otherPlayer.getPlayerRights().isDeveloperOrGreater(otherPlayer) && !otherPlayer.mode().isDarklord()) {
            player.message(" You are an Darklord, you only able to trade dark lord mode");
            return;

        }
        if (otherPlayer != null && otherPlayer.mode().isDarklord()  && !player.getPlayerRights().isDeveloperOrGreater(player) && !player.mode().isDarklord()) {
            player.message(otherPlayer.getUsername() + " are an Darklord. they stand alone");
            return;
    ...
    public void abortTrading() {
        if (state != TradeState.NONE) {
            ...
            Utils.sendDiscordInfoLog(player, "Player " + player.getUsername() + " aborted the trade returning items: " + Arrays.toString(container.toNonNullArray()), "trade");
        ...
        if (state == TradeState.TRADE_SCREEN) {
            ...
            if (state == TradeState.ACCEPTED_CONFIRM_SCREEN &&
                ...
                try {
                    ...
                    Utils.sendDiscordInfoLog(player, "Player " + player.getUsername() + " gave " + interactItems + " in a trade to " + partner.getUsername(), "trade");
                    tradeLogs.log(TRADE, "Player " + partner.getUsername() + " gave " + playerItems + " in a trade to " + player.getUsername());
                    Utils.sendDiscordInfoLog(player,"Player " + partner.getUsername() + " gave " + playerItems + " in a trade to " + player.getUsername(), "trade");
                    ...
                    Utils.sendDiscordInfoLog(player,"Player " + player.getUsername() + " (lvl " + player.skills().combatLevel() + ") and " + partner.getUsername() + " (lvl " + partner.skills().combatLevel() + ") trade value difference of " + Utils.insertCommasToNumber(String.valueOf(difference)) + " blood money.", "trade");
                    if (difference > 1_000_000) {
                        ...
                        Utils.sendDiscordInfoLog(player,GameServer.properties().discordNotifyId + " Player " + player.getUsername() + " has traded with Player " + partner.getUsername() + " with a value difference of greater than 1.000.000 blood money, this was possibly RWT.", "trade");
                    }
                } catch (Exception e) {
    ...
    public void handleItem(int id, int amount, int slot, ItemContainer from, ItemContainer to) {
       ...
       /* if (tradeItem.unnote().definition(World.getWorld()).pvpAllowed) {
        ...
        //ok bro its done let me find solve for jail command thanks bro!!! what do you want more ? just the website sided things i. highscores for iron/harcore and 4 vote points for claim

        //hiscores dont update for ironman/hardcore? //ill show
