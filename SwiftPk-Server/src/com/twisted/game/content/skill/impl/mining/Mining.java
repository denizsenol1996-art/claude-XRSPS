...
import com.twisted.game.action.impl.UnwalkableAction;
...
//dzone3
                    var totalXp = finalRock.xp;// calculate 2.5% of the weCatch's xp value per piece
                    if (!player.jailed()) {//dzone3
                        if (hat != null && hat.getId() == 12013) {
                            totalXp += 2.5; // add 2.5% XP per piece for each equipped angler item
                        }
                        if (top != null && top.getId() == 12014) {
                            totalXp += 2.5;
                        }
                        ...
                        player.skills().addXp(Skills.MINING, totalXp);
                        AchievementsManager.activate(player, Achievements.MINER_I, 1);
                        AchievementsManager.activate(player, Achievements.MINER_II, 1);
                        AchievementsManager.activate(player, Achievements.MINER_III, 1);
                        AchievementsManager.activate(player, Achievements.MINER_IV, 1);
