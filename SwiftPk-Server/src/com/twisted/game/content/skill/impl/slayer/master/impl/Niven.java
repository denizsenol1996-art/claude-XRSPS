 ...
 */
public class Niven extends PacketInteraction {
    ...
    private static void assignTask(Player player, String taskType) {
        ...
        if (numleft > 0) {
            player.getDialogueManager().start(new Dialogue() {
                ...
                protected void start(Object... parameters) {
                    send(DialogueType.NPC_STATEMENT, NIVEN, Expression.HAPPY, "You're still hunting " + Slayer.taskName(player.slayerTaskId()) + "; you have " + numleft + " to go.", "Come back when you've finished your task.");
                    setPhase(0);
                }
                ...
                protected void next() {
                    if (isPhase(0)) {
                        send(DialogueType.NPC_STATEMENT, NIVEN, Expression.HAPPY, "Come back to me when you've completed your task", "for a new one.");
                        setPhase(1);
                    } else if (isPhase(1)) {
                        stop();
                    }
        ...
        if(taskType.equalsIgnoreCase("boss")) {
            ...
            SlayerMaster.assign(player, NIVEN);
        }
        ...
        player.getDialogueManager().start(new Dialogue() {
            @Override
            protected void start(Object... parameters) {
                send(DialogueType.NPC_STATEMENT, NIVEN, Expression.HAPPY, "Ok, great. Your new task is to kill " + num + " " + Slayer.taskName(task.uid) + ".");
                setPhase(0);
            }
    ...
    private static void cancelForBloodMoney(Player player) {
        player.getDialogueManager().start(new Dialogue() {
            ...
            protected void next() {
                if (isPhase(0)) {
                    send(DialogueType.NPC_STATEMENT, NIVEN, Expression.HAPPY, "Certainly, just as a reminder, the fee is " + BM_CANCEL_FEE + " blood money.", "This action is irreversible.");
                    setPhase(1);
                } else if (isPhase(1)) {
                    ...
                    send(DialogueType.NPC_STATEMENT, NIVEN, Expression.HAPPY, "There you go. Your current slayer task has been cleared.", "Come speak to me when you're ready for a new one.");
                    setPhase(4);
                } else if (isPhase(4)) {
                    stop();
                } else if (isPhase(5)) {
                    send(DialogueType.NPC_STATEMENT, NIVEN, Expression.HAPPY, "Come speak to me when you have enough blood money.");
                    setPhase(4);
                }
            }
                ...
                if (isPhase(2)) {
                    if (option == 1) {
                        if (player.getInventory().contains(new Item(13307)) && player.getInventory().byId(13307).getAmount() >= BM_CANCEL_FEE) {
                            ...
                            send(DialogueType.NPC_STATEMENT, NIVEN, Expression.HAPPY, "Sorry, but it appears you do not have enough blood money", "to cover the cancellation fee.");
                            setPhase(5);
                        }
                    } else if (option == 2) {
                        stop();
