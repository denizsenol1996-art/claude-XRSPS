package com.twisted.game.content.skill.impl.firemaking;

import com.twisted.game.content.tasks.impl.Tasks;
import com.twisted.game.task.TaskManager;
import com.twisted.game.task.impl.TimedObjectSpawnTask;
...
import static com.twisted.util.ObjectIdentifiers.FIRE_26185;
...
public class LogLighting {
    ...
    public static boolean onItemOnItem(Player player, Item use, Item with) {
        for (LightableLog log : LightableLog.values()) {
            if ((use.getId() == TINDERBOX || with.getId() == TINDERBOX) && (use.getId() == log.id || with.getId() == log.id)) {
                ...
                boolean catchFire = player.getTimerRepository().has(TimerKey.FIRE_CATCHING);
    ...
    private static void burnComplete(Player player, LightableLog log, Tile finalTargTile, GroundItem spawnedItem) {
        ...
        player.getTimerRepository().register(TimerKey.FIRE_CATCHING, 3);
