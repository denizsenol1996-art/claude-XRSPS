package com.twisted.game.content.skill.impl.slayer.master.impl;
...
import static com.twisted.game.world.entity.mob.player.QuestTab.InfoTab.TASK_STREAK;
 ...
 */
public class Steve extends PacketInteraction {

    public static final int BM_CANCEL_FEE = 10_000;

    @Override
    public boolean handleNpcInteraction(Player player, Npc npc, int option) {
        if(option == 1) {
            if (npc.id() == STEVE_6799) {
    ...
    private static void assignTask(Player player, String taskType) {
        ...
        int masterId = player.getAttribOr(AttributeKey.SLAYER_MASTER, 0);

        if (numleft > 0 && masterId == STEVE_6799) {
            player.getDialogueManager().start(new Dialogue() {
                @Override
                protected void start(Object... parameters) {
                    send(DialogueType.NPC_STATEMENT, STEVE_6799, Expression.HAPPY, "You're still hunting " + Slayer.taskName(player.slayerTaskId()) + "; you have " + numleft + " to go.", "Come back when you've finished your task.");
                    setPhase(0);
                }
                ...
                protected void next() {
                    if (isPhase(0)) {
                        send(DialogueType.NPC_STATEMENT, STEVE_6799, Expression.HAPPY, "Come back to me when you've completed your task", "for a new one.");
                        setPhase(1);
                    } else if (isPhase(1)) {
                        stop();
                    }
        ...
        if(taskType.equalsIgnoreCase("low")) {
            SlayerMaster.assign(player, STEVE_6799);
        }
        ...
        player.getDialogueManager().start(new Dialogue() {
            @Override
            protected void start(Object... parameters) {
                send(DialogueType.NPC_STATEMENT, STEVE_6799, Expression.HAPPY, "Ok, great. Your new task is to kill " + num + " " + Slayer.taskName(task.uid) + ".");
                setPhase(0);
            }
    ...
    private static void taskOptions(Player player) {
        player.getDialogueManager().start(new Dialogue() {
            @Override
            protected void start(Object... parameters) {
                send(DialogueType.OPTION, DEFAULT_OPTION_TITLE,"I'd like a regular task, please.", "Nothing");
                setPhase(0);
            }
    ...
    private static void displayOptions(Player player) {
        player.getDialogueManager().start(new Dialogue() {
            @Override
            protected void start(Object... parameters) {
                send(DialogueType.OPTION, DEFAULT_OPTION_TITLE,  "I'd like a regular task, please.", "Can you cancel my current task for blood money?", "Nothing");
                setPhase(0);
            }
    ...
    private static void cancelForBloodMoney(Player player) {
        player.getDialogueManager().start(new Dialogue() {
            ...
            protected void next() {
                if (isPhase(0)) {
                    send(DialogueType.NPC_STATEMENT, STEVE_6799, Expression.HAPPY, "Certainly, just as a reminder, the fee is " + BM_CANCEL_FEE + " blood money.", "This action is irreversible.");
                    setPhase(1);
                } else if (isPhase(1)) {
                    ...
                    send(DialogueType.NPC_STATEMENT, STEVE_6799, Expression.HAPPY, "There you go. Your current slayer task has been cleared.", "Come speak to me when you're ready for a new one.");
                    setPhase(4);
                } else if (isPhase(4)) {
                    stop();
                } else if (isPhase(5)) {
                    send(DialogueType.NPC_STATEMENT, STEVE_6799, Expression.HAPPY, "Come speak to me when you have enough blood money.");
                    setPhase(4);
                }
            }
                ...
                if (isPhase(2)) {
                    if (option == 1) {
                        if (player.getInventory().contains(new Item(13307)) && player.getInventory().byId(13307).getAmount() >= BM_CANCEL_FEE) {
                            ...
                            send(DialogueType.NPC_STATEMENT, STEVE_6799, Expression.HAPPY, "Sorry, but it appears you do not have enough blood money", "to cover the cancellation fee.");
                            setPhase(5);
                        }
                    } else if (option == 2) {
                        stop();
