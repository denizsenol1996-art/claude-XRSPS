...
import static com.twisted.util.ItemIdentifiers.SUPER_ANTIFIRE_POTION4;
import static com.twisted.util.NpcIdentifiers.*;
import static com.twisted.util.NpcIdentifiers.CAVE_KRAKEN;
...
public class SlayerKey {
    ...
    public void drop(Npc npc) {//plateau 33
        var task_id = player.<Integer>getAttribOr(SLAYER_TASK_ID, 0);
        var task = SlayerCreature.lookup(task_id);
        int random = Utils.random(25);
        if (task != null && Slayer.creatureMatches(player, npc.id())) {
            if (random > 23 && random < 25 && player.slayerTaskAmount() > 0) {//take another task and pickup key
                ...
                if (npc.id() == 494 || npc.id() == CAVE_KRAKEN || npc.id() == KRAKEN || npc.id() == KRAKEN_6640 || npc.id() == KRAKEN_6656) {
                    if (player.getSlayerRewards().getUnlocks().containsKey(SlayerConstants.SLAYER_KEY_PERK)) {//fix1
                        player.getInventory().addOrDrop(new Item(SLAYER_KEY));
                        player.message(Color.PURPLE.wrap("A slayer key added to your inventory, you have now collected a total of " + keysReceived + " slayer keys."));
                    } else {
                        GroundItem groundItem = new GroundItem(new Item(SLAYER_KEY), player.tile(), player);//update4.22
                    ...
                    if (player.getSlayerRewards().getUnlocks().containsKey(SlayerConstants.SLAYER_KEY_PERK)) {//fix1
                        ...
                        GroundItem groundItem = new GroundItem(new Item(SLAYER_KEY), player.tile(), player);
                        GroundItemHandler.createGroundItem(groundItem);
                        player.message(Color.PURPLE.wrap("A slayer key appeared, you have now collected a total of " + keysReceived + " slayer keys."));
                    }
