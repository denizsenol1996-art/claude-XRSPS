...
import static com.twisted.util.ItemIdentifiers.ATTACK_CAPET;
...
public class CyclopsRoom {
    public static boolean hasFreeFeeItems(Player player) {
        return player.getEquipment().hasAt(EquipSlot.CAPE, ATTACK_CAPE)
            || player.getEquipment().hasAt(EquipSlot.CAPE, ATTACK_CAPET);
        ...
        if (basement) {
            TaskManager.submit(new Task("handle_time_spent_task", 1) {
                ...
                protected void execute() {
                    ...
                    if (internalTimer-- == 0) {
                        if (player.inventory().remove(new Item(WARRIOR_GUILD_TOKEN, 10), true) || hasFreeFeeItems(player)) {
                            if(hasFreeFeeItems(player)){
                                player.message("<col=804080>Since you are wearing a cape, no tokens will be remove.");
