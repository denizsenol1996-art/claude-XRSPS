...
import static com.twisted.util.ObjectIdentifiers.DOOR_24306;
import static com.twisted.util.ObjectIdentifiers.DOOR_24309;
...
public class WarriorDoubleDoor extends PacketInteraction {
    ...
    private static void door(Player player) {
        ...
        if (player.tile().level == 0) {
            ...
            if (!player.inventory().contains(new Item(WARRIOR_GUILD_TOKEN, 100))&& !CyclopsRoom.hasFreeFeeItems(player) && player.tile().x == 2846) {
                player.unlock();
