package com.twisted.game.content.areas.wilderness.content;
...
import com.twisted.game.content.areas.wilderness.content.activity.WildernessActivityManager;
import com.twisted.game.content.areas.wilderness.content.activity.impl.EdgevileActivity;
import com.twisted.game.content.areas.wilderness.content.activity.impl.PureActivity;
import com.twisted.game.content.areas.wilderness.content.activity.impl.ZerkerActivity;
...
import com.twisted.game.content.tasks.Requirements;
...
import static com.twisted.util.ItemIdentifiers.PK_TICKET;
...
public class PlayerKillingRewards {
    ...
    public static void reward(Player killer, Player target, boolean valid) {
        ...
        try {
            ...
            if (valid) {
                ...
                if (ServerEvent.isBloodMoneyBoost()) {
                    bm *= 2;
                }

                GroundItem bloodMoney = new GroundItem(new Item(BLOOD_MONEY, bm), target.tile(), killer);
                GroundItem pkTicket = new GroundItem(new Item(PK_TICKET, 1), target.tile(), killer);
                GroundItem pkTicket2 = new GroundItem(new Item(PK_TICKET, 2), target.tile(), killer);
                GroundItem pkTicket3 = new GroundItem(new Item(PK_TICKET, 3), target.tile(), killer);
                GroundItem pkTicket5 = new GroundItem(new Item(PK_TICKET, 5), target.tile(), killer);
                GroundItemHandler.createGroundItem(bloodMoney);
                if(killstreak > 5 && killstreak < 10){
                    GroundItemHandler.createGroundItem(pkTicket2);
                    killer.message(Color.RED.tag() + "<shad=0>[PKT]</col></shad> " + Color.BLUE.tag() + "You earn " + Color.VIOLET.tag() + "2 pk ticket " + Color.BLUE.tag() + " after killing " + Color.VIOLET.tag() + "" + target.getUsername() + "" + Color.BLUE.tag() + "!");

                } else if(killstreak > 10 && killstreak < 20){
                    GroundItemHandler.createGroundItem(pkTicket3);
                    killer.message(Color.RED.tag() + "<shad=0>[PKT]</col></shad> " + Color.BLUE.tag() + "You earn " + Color.VIOLET.tag() + "3 pk ticket " + Color.BLUE.tag() + " after killing " + Color.VIOLET.tag() + "" + target.getUsername() + "" + Color.BLUE.tag() + "!");
                } else if(killstreak > 20){
                    GroundItemHandler.createGroundItem(pkTicket5);
                    killer.message(Color.RED.tag() + "<shad=0>[PKT]</col></shad> " + Color.BLUE.tag() + "You earn " + Color.VIOLET.tag() + "5 pk ticket " + Color.BLUE.tag() + " after killing " + Color.VIOLET.tag() + "" + target.getUsername() + "" + Color.BLUE.tag() + "!");

                } else {
                    killer.message(Color.RED.tag() + "<shad=0>[PKT]</col></shad> " + Color.BLUE.tag() + "You earn " + Color.VIOLET.tag() + "1 pk ticket " + Color.BLUE.tag() + " after killing " + Color.VIOLET.tag() + "" + target.getUsername() + "" + Color.BLUE.tag() + "!");
                    GroundItemHandler.createGroundItem(pkTicket);
                ...
                if(whosKeysAreThese && World.getWorld().rollDie(250,1)) {
                    killer.inventory().addOrBank(new Item(KEY_OF_BOXES));
    ...
    private static void checkForTask(Player player) {
        if (CombatConstants.wearingTorags(player) && Requirements.hasZulrahPet(player)) {
        ...
        if (Requirements.bmRisk(player) > 20_000 && !Prayers.usingPrayer(player, Prayers.PROTECT_ITEM) || Requirements.bmRisk(player) > 20_000 && !Prayers.usingPrayer(player, Prayers.PROTECT_ITEM_2)) {
