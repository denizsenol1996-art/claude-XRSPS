package com.twisted.game.content.duel;
...
import com.twisted.game.world.entity.mob.player.commands.impl.owner.ToggleDuelCommand;
...
public class Dueling {
    ...
    public static boolean stake_not_started(Mob entity, Mob other) {
        ...
        return entity.getTimerRepository().has(TimerKey.STAKE_COUNTDOWN);
    }
    ...
    public void requestDuel(Player t_) {
        ...
        if (ToggleDuelCommand.duelDisabled) {//updateox
            player.message("@red@Duel is currently disabled.");
            return;
        }
...
*/
    ...
    private void askDuelSettings() {
        //Player has a pet out, prevent duel from starting
        if(player.pet() != null) {
            player.message("You can't duel when having pets out.");
            return;
        }
    ...
    private void heal_player(Player player) {
        ...
        Venom.cure(2, player);
        player.getTimerRepository().cancel(TimerKey.FROZEN); //Remove frozen timer key
        player.getTimerRepository().cancel(TimerKey.STUNNED); //Remove stunned timer key
        player.getTimerRepository().cancel(TimerKey.TELEBLOCK); //Remove teleblock timer key
        player.getTimerRepository().cancel(TimerKey.TELEBLOCK_IMMUNITY); //Remove the teleblock immunity timer key
        player.setRunningEnergy(100.0, true); //Set energy to 100%
        player.setSpecialActivated(false); //Disable special attack
        player.getTimerRepository().cancel(TimerKey.COMBAT_LOGOUT); //Remove combat logout timer key
    ...
    private void startDuel(Tile telePos) {
        ...
        if (playerHasOverload) {
            ...
            player.getTimerRepository().cancel(TimerKey.POTION);
        ...
        player.getTimerRepository().cancel(TimerKey.SOTD_DAMAGE_REDUCTION);
        opponent.getTimerRepository().cancel(TimerKey.SOTD_DAMAGE_REDUCTION);
        ...
        player.getTimerRepository().register(TimerKey.STAKE_COUNTDOWN, 11);
        opponent.getTimerRepository().register(TimerKey.STAKE_COUNTDOWN, 11);
    ...
    public void onDeath() {
        ...
        if (validate(player, opponent, null, DuelState.STARTING_DUEL, DuelState.IN_DUEL)) {
            ...
            try {
                ...
                Utils.sendDiscordInfoLog(player, "Player " + opponent.getUsername() + " got " + playerItems + " from winning a duel against " + player.getUsername(), "stake");
                stakeLogs.log(STAKE, "Player " + opponent.getUsername() + " already had " + interactItems + " from winning a duel against " + player.getUsername());
                Utils.sendDiscordInfoLog(player, "Player " + opponent.getUsername() + " already had " + interactItems + " from winning a duel against " + player.getUsername(), "stake");
                ...
                Utils.sendDiscordInfoLog(player,"Player " + player.getUsername() + " (lvl " + player.skills().combatLevel() + ") and " + opponent.getUsername() + " (lvl " + opponent.skills().combatLevel() + ") duel stake value difference of " + Utils.insertCommasToNumber(String.valueOf(difference)) + " " + currencyType, "stake");
                if (difference > 1_000_000) {
                    ...
                    Utils.sendDiscordInfoLog(player,GameServer.properties().discordNotifyId + " Player " + opponent.getUsername() + " won a stake against Player " + player.getUsername() + " with a value difference of greater than 1,000,000 " + currencyType + ", this was possibly RWT.", "stake");
                }
