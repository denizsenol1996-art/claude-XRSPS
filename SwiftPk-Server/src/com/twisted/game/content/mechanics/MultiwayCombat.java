...
import static com.twisted.util.ItemIdentifiers.BLURITE_ORE;
import static com.twisted.util.NpcIdentifiers.THE_NIGHTMARE_9430;
...
public class MultiwayCombat {
    ...
    public static boolean includes(Mob mob) {
        ...
        if (mob.isNpc()) {//upos
            if (mob.getAsNpc().isWorldBoss() || mob.getAsNpc().id() == 13391 ||mob.getAsNpc().id() == 8633 || mob.getAsNpc().id() == 15003|| mob.getAsNpc().isCombatDummy() || mob.getAsNpc().id() == THE_NIGHTMARE_9430) {
                return true;
            }
            if(mob.tile().region() == 9369 &&  mob.getAsNpc().id() == 6613 || mob.getAsNpc().id() == NpcIdentifiers.SCORPIAS_GUARDIAN) {
                return true;
            }
        }
    ...
    private static final List<Integer> REGIONS = Arrays.asList(
        ...
        12869, 12613, 13125, 13122, 13123, 12611, 12612, //TOB
    ...
    private static final List<Integer> CHUNKS = Arrays.asList(
        ...
        //nex cage
        19988816,20054353,19988817,20054352,
