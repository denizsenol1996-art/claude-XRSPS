package com.twisted.game.content.consumables.potions;
...
public class Potions {
    ...
    public static void consume(Player player, Potion potion, int id) {
        ...
        if(potion == Potion.DIVINE_RANGING_POTION || potion == Potion.DIVINE_SUPER_COMBAT_POTION){//up02
        ...
        if((potion == Potion.OVERLOAD_POTION)) {//upos
            if (!Tile.inRaid1(player) && !Tile.inRaid2(player) && !player.tile().inArea(AreaConstants.THEATRE)) {
    ...
    private static void deductDose(Player player, Potion potion, int id) {
        if (potion.isLastDose(id)) {
            boolean giveEmptyVials = player.<Boolean>getAttribOr(AttributeKey.GIVE_EMPTY_POTION_VIALS, true);
