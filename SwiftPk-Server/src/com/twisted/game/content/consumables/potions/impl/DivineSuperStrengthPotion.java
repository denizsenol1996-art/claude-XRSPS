...
public class DivineSuperStrengthPotion {
    ...
    public static void setTimer(Player player) {
        player.putAttrib(AttributeKey.DIVINE_SUPER_STRENGTH_POTION_TASK_RUNNING, true);
        TaskManager.submit(new Task("DivineSuperStrengthPotionTask", 1, false) {
            ...
            protected void execute() {
                ...
                if (!player.isRegistered() || player.dead() || ticks == 0) {
                    stop();
                    player.clearAttrib(AttributeKey.DIVINE_SUPER_STRENGTH_POTION_TASK_RUNNING);
                    return;
                }

                if (potionEffectActive) {
                    player.getPacketSender().sendEffectTimer((int) Utils.ticksToSeconds(ticks), EffectTimer.DIVINE_SUPER_STRENGTH_POTION);
                    ticks--;
                    player.putAttrib(AttributeKey.DIVINE_SUPER_STRENGTH_POTION_TICKS, ticks--);
                    if (ticks == 0) {
                        player.putAttrib(AttributeKey.DIVINE_SUPER_STRENGTH_POTION_TASK_RUNNING, false);
                        player.putAttrib(AttributeKey.DIVINE_SUPER_STRENGTH_POTION_EFFECT_ACTIVE, false);
                        player.putAttrib(AttributeKey.DIVINE_SUPER_STRENGTH_POTION_TICKS, 0);
