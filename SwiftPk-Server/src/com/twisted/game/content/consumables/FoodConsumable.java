package com.twisted.game.content.consumables;
...
public class FoodConsumable {
    ...
    private static void eat(Player player, Food food) {
        ...
        if (food == Food.KARAMBWAN) {
            if (player.getTimerRepository().has(TimerKey.KARAMBWAN) || player.dead() || player.hp() < 1) {
                player.debugMessage("Your Karambwan timer is still active, " + player.getTimerRepository().asSeconds(TimerKey.KARAMBWAN) + " remaining.");
                return;
            }
        } else {
            if (player.getTimerRepository().has(TimerKey.FOOD) || player.dead() || player.hp() < 1) {
                player.debugMessage("Your food timer is still active, " + player.getTimerRepository().asSeconds(TimerKey.FOOD) + " remaining.");
                return;
            }
        }
        ...
        if (!Prayers.usingPrayer(player, Prayers.METABOLISE)) {
            player.getTimerRepository().register(TimerKey.FOOD, 3);
        } else {
            if (ticks >= 2)
                ticks = ticks - 1;

            player.getTimerRepository().extendOrRegister(TimerKey.FOOD, ticks);
        }

        player.getTimerRepository().extendOrRegister(TimerKey.COMBAT_ATTACK, 5);
        ...
        if (!food.effect) {
            ...
            if (food == Food.BEER) {
                ...
                player.getTimerRepository().register(TimerKey.POTION, 3); // Register the potion timer (karambwan blocks pots)
