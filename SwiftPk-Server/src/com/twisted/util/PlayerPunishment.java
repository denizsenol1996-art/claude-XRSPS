...
public class PlayerPunishment {
    ...
    public static void addToIPReferList(String IP) {
        IpRefer.add(IP);
        addIPToFile(IP, "./data/saves/iprefer.txt");
    }
    public static boolean ipRefer(String IP) {
        return IpRefer.contains(IP);
    }
    public static void referIps() {
        try {
            try (BufferedReader in = new BufferedReader(new FileReader("./data/saves/iprefer.txt"))) {
                String data = null;
    ...
    public static void macReferUsers() {
        File file = new File("./data/saves/refer.txt");
        if (!file.exists()) {
            try {
                file.createNewFile();
        ...
        try (BufferedReader in = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = in.readLine()) != null) {
                if (!referClaim.contains(line) && !line.isEmpty()) {
                    referClaim.add(line);
                }
            }
        } catch (IOException e) {
    ...
    public static void addReferReclaim(String address) {
        if (!referClaim.contains(address) && !address.isEmpty()) {
            referClaim.add(address);
            updateReferFile();
        }
    }
    ...
    private static void updateReferFile() {
        try (BufferedWriter out = new BufferedWriter(new FileWriter("./data/saves/refer.txt"))) {
            for (String mac : referClaim) {
                out.write(mac);
                out.newLine();
            }
        } catch (IOException e) {
    ...
    public static boolean referBlock(String address) {
        return referClaim.contains(address);
    }
}
