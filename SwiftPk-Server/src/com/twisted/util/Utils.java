...
import static com.twisted.util.DiscordWebhook.EmbedObject.buildEmbedMessageOnly;
...
public class Utils {
    ...
    public static final long HAZY_EPOCH = 1699574400000L;
    private static final AtomicLong increment = new AtomicLong(0L);

    public static long generateUUID() {
        long timestamp = System.currentTimeMillis();
        long generation = (timestamp - HAZY_EPOCH) << 22;
    ...
    public static void sendDiscordInfoLog(Player player, String text, String type) {
        if (GameServer.properties().enableDiscordLogging) {
            GameEngine.getInstance().submitDiscord(() -> {
                try {
                    switch (type) {
                        case "command" ->
                            executeHook(GameServer.getCommandWebHook(), "Command-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "warning" ->
                            executeHook(GameServer.getWarningWebHook(), "Warning-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "trade" ->
                            executeHook(GameServer.getTradeWebHook(), "Trade-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "stake" ->
                            executeHook(GameServer.getStakeWebHook(), "Stake-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "chat" ->
                            executeHook(GameServer.getChatWebHook(), "Chat-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "pm" ->
                            executeHook(GameServer.getPmWebHook(), "Private-Message-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "cc" ->
                            executeHook(GameServer.getClanChatWebHook(), "Clan-Chat-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "npcdrops" ->
                            executeHook(GameServer.getNpcDropsWebHook(), "NPC-Drops-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "playerdrops" ->
                            executeHook(GameServer.getPlayerDropsWebHook(), "Player-Drops-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "pickups" ->
                            executeHook(GameServer.getPickupsWebHook(), "Pickups-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "login" ->
                            executeHook(GameServer.getLoginWebHook(), "Login-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "logout" ->
                            executeHook(GameServer.getLogoutWebHook(), "Logout-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "sanctions" ->
                            executeHook(GameServer.getSanctionsWebHook(), "Sanctions-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "shops" ->
                            executeHook(GameServer.getShopsWebHook(), "Shops-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "playerdeaths" ->
                            executeHook(GameServer.getPlayerDeathsWebHook(), "Player-Deaths-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "passwordchange" ->
                            executeHook(GameServer.getPasswordChangeWebHook(), "Password-Change-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "tournaments" ->
                            executeHook(GameServer.getTournamentWebHook(), "Tournaments-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "referrals" ->
                            executeHook(GameServer.getReferralsWebHook(), "Referrals-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "achievements" ->
                            executeHook(GameServer.getAchievementsWebHookWebHook(), "Achievements-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "trading_post_sales" ->
                            executeHook(GameServer.getTradingPostSalesWebHook(), "Trading-Post-Sales-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "trading_post_purchases" ->
                            executeHook(GameServer.getTradingPostPurchasesWebHook(), "Trading-Post-Purchases-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "raids" ->
                            executeHook(GameServer.getRaidsWebHook(), "Raids-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "starter_box_received" ->
                            executeHook(GameServer.getStarterBoxWebHook(), "Starter-Box-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "clan_box_opened" ->
                            executeHook(GameServer.getClanBoxWebHook(), "Clan-Box-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "gamble" ->
                            executeHook(GameServer.getGambleWebHook(), "Gamble-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "box_and_tickets" ->
                            executeHook(GameServer.getBoxAndTicketsWebHookUrl(), "Box-And-Tickets-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                        case "fpk_merk" ->
                            executeHook(GameServer.getFpkMerkwebHookURL(), "Fpk-Merk-Logs", text.replaceAll("\"", ""), player.getUsername(), player.getHostAddress());
                    }
                } catch (IOException e) {
    ...
    public static void sendDiscordInfoLog(Player player, String text) {
        sendDiscordInfoLog(player, text, "info");
    }
    ...
    public static void executeHook(final DiscordWebhook webhook, final String title, final String message, final String username, final String sessionIp) throws IOException, URISyntaxException {
        if (GameServer.properties().enableDiscordLogging) {
            GameEngine.getInstance().submitDiscord(() -> {
                DiscordWebhook.EmbedObject embed = buildEmbedMessageOnly(RED, title, message, username, sessionIp);
    ...
    public static Player makebot(String user) {
        Player player = new Session(null).getPlayer();
