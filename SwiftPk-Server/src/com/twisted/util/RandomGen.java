...
public final class RandomGen {
    ...
    public <T> T random(List<T> list) {
        List<T> collect = list.stream().filter(Objects::nonNull).collect(Collectors.toList());
        if (collect.isEmpty())
            return null;
        return collect.get((int) (random.nextDouble() * collect.size()));
    }

    public static void main(String[] args) {
        List<Integer> randomTest = Arrays.asList();
        RandomGen r = new RandomGen();
        for (int i = 0; i < 1000; i++) {

            // bound to throw a error once in 1k
            r.random(randomTest);
        }
    }
