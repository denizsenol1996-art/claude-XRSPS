package com.twisted.cache.util;

import java.util.Arrays;

public class WhirlpoolHashing {
    ...
    private static WhirlpoolHashing INSTANCE;

    public WhirlpoolHashing() {
    }
    ...
    public static synchronized byte[] hash(byte[] input) {
        byte[] digest = new byte[64];
        INSTANCE.NESSIEinit();
        INSTANCE.NESSIEadd(input, (long)(input.length * 8));
        INSTANCE.NESSIEfinalize(digest);
        return digest;
    }
