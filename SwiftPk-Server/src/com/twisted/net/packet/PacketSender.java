package com.twisted.net.packet;
...
import com.twisted.game.world.entity.mob.player.PlayerInteractingOption;
...
import com.twisted.net.packet.outgoing.message.ComponentVisibility;
...
public final class PacketSender {
    ...
    public PacketSender updateSkill(int skill, int level, int xp) {
        ...
        final Prestige prestige = Prestige.forSkill(Skill.fromId(skill));
        final int state;
        if (prestige != null)
            state = player.<Integer>getAttribOr(prestige.key, 0);
        else
            state = 0;

        out.put(skill)
            .putInt(xp, ByteOrder.MIDDLE)
            .put(level)
            .put(state);
        player.getSession().write(out);
        return this;
        ...
        for (int i = 0; i < GameConstants.SIDEBAR_INTERFACE.length; i++) {
            ...
            if (tab == 5) {
                if (player.getAttribOr(AttributeKey.RUINOUS_PRAYERS, false)) {
                    updateTab(1, 5);
                    player.getInterfaceManager().setSidebar(tab, 24493);
                } else {
                    updateTab(0, 5);
                    player.getInterfaceManager().setSidebar(tab, 5608);
                }
            }
            if (tab == 6) {
    ...
    public PacketSender sendObject(GameObject object) {
        sendMapPacket(object.tile().getX(), object.tile().getY(), object.tile().getZ());
     ...
     *
     * @param tile        The starting location.
     * @param offset      The finishing location.
 ...
 /*   public PacketSender sendProjectile(Tile tile, Tile offset, int angle, int speed, int id, int startHeight, int endHeight, int lockon, int delay, int slope, int radius) {
        sendMapPacket(tile.getX(), tile.getY(), tile.getZ());
    ...
    public PacketSender sendProjectile(int startX, int startY, int destX, int destY, int angle, int duration, int gfxMoving, int startHeight, int endHeight, int lockon, int startDelay, int slope, int startDistanceOffset) {
        sendMapPacket(startX, startY, player.getZ());
        if (player.getSession() != null)
            player.getSession().write(new PacketBuilder(117)
                .put(angle).put(destY)
                .put(destX)
                .putShort(lockon)
                .putShort(gfxMoving)
