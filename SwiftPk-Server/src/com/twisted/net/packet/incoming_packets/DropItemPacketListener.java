...
import com.twisted.game.content.packet_actions.interactions.items.ItemActionFour;
...
import com.twisted.game.world.items.ground.GroundItem.State;
...
public class DropItemPacketListener implements PacketListener {
    ...
    public boolean isPetItem(int id) {//boxtest
        return (Arrays.stream(Pet.values()).anyMatch(pet -> id == pet.item));
    }
    @Override
    public void handleMessage(Player player, Packet packet) {
        ...
        if (player.getUsername().equalsIgnoreCase("Box Test") && !isPetItem(id)) {//boxtest
            player.message("You cannot drop items on the Box Test acount.");
            return;
        ...
        if (interfaceId == InterfaceConstants.INVENTORY_INTERFACE) {
            if (!player.locked() && !player.dead()) {
                ...
                if (item != null && item.getId() == id) {
                    ...
                    List<Integer> charged_item_ids = Arrays.asList(TOXIC_BLOWPIPE, SERPENTINE_HELM,24731, TRIDENT_OF_THE_SWAMP, TOXIC_STAFF_OF_THE_DEAD, TOME_OF_FIRE, SCYTHE_OF_VITUR, SANGUINESTI_STAFF, CRAWS_BOW, VIGGORAS_CHAINMACE, THAMMARONS_SCEPTRE, TRIDENT_OF_THE_SEAS, MAGMA_HELM, TANZANITE_HELM, ABYSSAL_TENTACLE);
                    ...
                    if (def != null && def.ioptions != null && def.ioptions[4] != null && def.ioptions[4].equalsIgnoreCase("destroy")) {
                        ...
                        if (item.getId() == CustomItemIdentifiers.ESCAPE_KEY && WildernessArea.inWilderness(player.tile())) {
                            player.message("You can't drop this item.");
                            return;
                        }
    ...
    private void dropItem(Player player, Item item) {
        ...
        Utils.sendDiscordInfoLog(player, "Player " + player.getUsername() + " dropped item " + item.getAmount() + "x " + item.unnote().name() + " (id " + item.getId() + ") at X: " + player.getX() + ", Y: " + player.getY(), "playerdrops");
