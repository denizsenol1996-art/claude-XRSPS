...
public class PacketEncoder extends MessageToByteEncoder<Packet> {
    ...
    protected void encode(ChannelHandlerContext ctx, Packet packet, ByteBuf out) {
        ...
        // Check for invalid size and log error
        if (size == -3) {
            logger.error("{PacketEncoder} Opcode {} is not whitelisted.", opcode);
            return;
        }
        ...
        if (currSize != expectedSize) {
            logger.error("{PacketEncoder} Opcode {} has expected size {} but is actually {}.", opcode, expectedSize, currSize);
            return;
        }
