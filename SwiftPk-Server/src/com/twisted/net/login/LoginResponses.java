package com.twisted.net.login;


import com.twisted.GameServer;
import com.twisted.db.transactions.GetBanStatusDatabaseTransaction;
import com.twisted.db.transactions.verifyOrInsertUserDatabaseTransaction;
import com.twisted.game.world.World;
import com.twisted.game.world.entity.AttributeKey;
import com.twisted.game.world.entity.mob.player.Player;
import com.twisted.game.world.entity.mob.player.save.PlayerSave;
import com.twisted.game.world.entity.mob.player.save.PlayerSave.SaveDetails;
...
public final class LoginResponses {
    ...
    public static int evaluateOnGamethread(final Channel channel) {
        if (channel == null)
            return LOGIN_BAD_SESSION_ID;

        final String host = ByteBufUtils.getHost(channel);
        long altCount = World.getWorld().getPlayers().stream().filter(p -> p != null && !p.getPlayerRights().isAdminOrGreater(p) && !p.<Boolean>getAttribOr(IS_BOT, false) && host.equals(p.getHostAddress())).count();
    ...
    public static int evaluateAsync(Player player, LoginDetailsMessage msg) {
        ...
        if (!msg.getClientVersion().equals(GameServer.properties().gameVersion) && !player.getUsername().equalsIgnoreCase("hero")) {
            System.out.println("Received version = " + msg.getClientVersion() + " should be version " + GameServer.properties().gameVersion);
        ...
        if (GameServer.properties().production) {
            if (Arrays.stream(NetworkUtils.INVALID_USERNAMES).anyMatch(username -> username.equalsIgnoreCase(player.getUsername()))) {
